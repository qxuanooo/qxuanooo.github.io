$(function(){
    ...
    var musicImgsData = ['../图片/music-IV.jpg','../图片/music-Merry.png','../图片/music-theme.jpg']    // 图片地址数组
    var musicNameData = ['IV','Merry Christmas Mr. Lawrence','theme of SSS'];                   // 歌曲名数组
    var artistNameData = ['AK,Mapps','坂本龍一','ANANT-GARDE EYES,Key Sounds Label']            // 创作歌手数组
    var musicUrls=['../music/AK,Mapps - IV.mp3','../music/坂本龍一 - Merry Christmas Mr. Lawrence.mp3','../music/ANANT-GARDE EYES,Key Sounds Label - theme of SSS.mp3'];// 歌曲mp3数组
    ...
    function playPause(){
        if(audio.paused){
            ...
            playPauseBtn.attr('class','btn play-pause icon-Playerpause iconfont') // 显示暂停图标
            checkBuffering(); // 检测是否需要缓冲
            audio.play();     // 播放
        }else{
            playerContent1.removeClass('active'); // 内容栏下移
            musicImgs.removeClass('active');      // 左侧图片停止旋转等动画效果
            playPauseBtn.attr('class','btn play-pause icon-bofangqi-bofang iconfont'); // 显示播放按钮
    ...
    function selectTrack(flag){
        ...
        if( flag == 0 ){
            playPauseBtn.attr('class','btn play-pause icon-bofangqi-bofang iconfont'); // 显示播放图标
        }else{
